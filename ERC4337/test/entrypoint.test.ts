...
describe("EntryPoint", function () {
 ...
 describe('Stake Management', () => {
  ...
  describe('with deposit', () => {
   ...
   before(async () => {
    ...
    wallet = await new OperaSmartWallet__factory(ethersSigner).deploy(entryPoint.address, owner, guardians)
 ...
 describe('#simulateValidation', () => {
  ...
  before(async () => {
   wallet1 = await new OperaSmartWallet__factory(ethersSigner).deploy(entryPoint.address, await walletOwner1.getAddress(), guardians)
  })
 ...
 describe('without paymaster (account pays in eth)', () => {
  ...
  describe('create account', () => {
   ...
   it('should reject create if sender address is wrong', async () => {
    ...
    const op = await fillAndSign({
     initCode: WalletConstructor(entryPoint.address, walletOwner.address, guardians),
     verificationGas: 2e6,
     sender: '0x'.padEnd(42, '1')
    }, walletOwner, entryPoint)
   ...
   it('should succeed to create account after prefund', async () => {
    ...
    const preAddr = await entryPoint.getSenderAddress(WalletConstructor(entryPoint.address, walletOwner.address, guardians), 0)
    await fund(preAddr)
  ...
  describe('batch multiple requests', () => {
   ...
   before('before', async () => {
    ...
    wallet1 = await entryPoint.getSenderAddress(WalletConstructor(entryPoint.address, walletOwner1.address, guardians), 0)
    wallet2 = await new OperaSmartWallet__factory(ethersSigner).deploy(entryPoint.address, walletOwner2.address, guardians)
    await fund(wallet1)
 ...
 describe('with paymaster (account with no eth)', () => {
  ...
  it('should fail if paymaster has no deposit', async function () {
   const op = await fillAndSign({
    ...
    callData: walletExecFromEntryPoint.data,
    initCode: WalletConstructor(entryPoint.address, wallet2Owner.address, guardians),
