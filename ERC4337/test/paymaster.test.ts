 ...
 describe('using TokenPaymaster (account pays in paymaster tokens)', () => {
  ...
  describe('create account', () => {
   ...
   it('account should pay for its creation (in tst)', async function () {
    ...
    const walletAddr = await entryPoint.getSenderAddress(WalletConstructor(entryPoint.address, walletOwner.address, guardians), 0)
   ...
   it('batched request should each pay for its share', async () => {
    ...
    for (let i = 0; i < 4; i++) {
     const aWallet = await new OperaSmartWallet__factory(ethersSigner).deploy(entryPoint.address, await walletOwner.getAddress(), guardians)
   ...
   describe('grief attempt', () => {
    ...
    before(async () => {
     wallet2 = await new OperaSmartWallet__factory(ethersSigner).deploy(entryPoint.address, await walletOwner.getAddress(), guardians)
