...
describe("OperaSmartWallet", function () {
 ...
 it('owner should be able to call transfer', async () => {
  const wallet = await new OperaSmartWallet__factory(ethers.provider.getSigner()).deploy(entryPoint, accounts[0], guardians)
 ...
 describe('#validateUserOp', () => {
  ...
  before(async () => {
   ...
   wallet = await new OperaSmartWallet__factory(await ethers.getSigner(entryPoint)).deploy(entryPoint, walletOwner.address, guardians)
